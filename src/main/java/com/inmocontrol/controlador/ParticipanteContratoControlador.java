...
public class ParticipanteContratoControlador {
	...
	public ResponseEntity<RespuestaExito<List<ParticipanteContratoEntidad>>> consultar(
			@RequestParam(required = false) UUID personaId, @RequestParam(required = false) UUID tipoParticipanteId,
			@RequestParam(required = false) UUID contratoId) {

		ParticipanteContratoDTO dto = new ParticipanteContratoDTO.Builder().personaId(personaId)
				.tipoParticipanteId(tipoParticipanteId).contratoId(contratoId).build();

		List<ParticipanteContratoEntidad> resultado;
		boolean tieneFiltros = personaId != null || tipoParticipanteId != null || contratoId != null;

		if (tieneFiltros) {
			ConsultarParticipanteContratoPorFiltrosFachada fachadaFiltros = new ConsultarParticipanteContratoPorFiltrosFachadaImpl();
			resultado = fachadaFiltros.ejecutar(dto);
			return ResponseEntity
					.ok(RespuestaExito.crear("Participantes de contrato filtrados obtenidos exitosamente", resultado));
		} else {
			ConsultarParticipanteContratoTodosFachada fachadaTodos = new ConsultarParticipanteContratoTodosFachadaImpl();
			resultado = fachadaTodos.ejecutar();
			return ResponseEntity
					.ok(RespuestaExito.crear("Participantes de contrato obtenidos exitosamente", resultado));
		}
	}
	...
	public ResponseEntity<RespuestaExito<Void>> eliminar(@PathVariable UUID id) {
		...
		fachada.ejecutar(dto);
		return ResponseEntity.ok(RespuestaExito.crear("Participante de contrato eliminado exitosamente"));
	}
}
