...
public class ContratoControlador {
	...
	@GetMapping
	public ResponseEntity<RespuestaExito<List<ContratoEntidad>>> consultarTodos() {
		ConsultarContratoTodosFachada fachada = new ConsultarContratoTodosFachadaImpl();
		List<ContratoEntidad> resultado = fachada.ejecutar(null);
	...
	@GetMapping("/buscar")
	public ResponseEntity<RespuestaExito<List<ContratoEntidad>>> consultarPorFiltros(
			@RequestParam(required = false) String codigoContrato, @RequestParam(required = false) Boolean esActivo,
			@RequestParam(required = false) UUID propiedadId, @RequestParam(required = false) String fechaInicio,
			@RequestParam(required = false) String fechaFin) {
		ContratoDTO dto = new ContratoDTO.Builder().codigoContrato(codigoContrato).esActivo(esActivo)
				.propiedad(propiedadId != null ? new PropiedadDTO.Builder().id(propiedadId).build() : null).build();
		ConsultarContratoPorFiltrosFachada fachada = new ConsultarContratoPorFiltrosFachadaImpl();
		List<ContratoEntidad> resultado = fachada.ejecutar(dto);
		return ResponseEntity.ok(RespuestaExito.crear("Contratos filtrados obtenidos exitosamente", resultado));
	}
	...
	public ResponseEntity<RespuestaExito<ContratoEntidad>> registrar(@RequestBody ContratoDTO dto) {
		...
		return ResponseEntity.status(201).body(RespuestaExito.crear("Contrato registrado exitosamente", resultado));
	}
