...
public class TipoParticipanteControlador {
	...
	public ResponseEntity<RespuestaExito<List<TipoParticipanteEntidad>>> consultar(
		...
		if (tieneFiltros) {
			ConsultarTipoParticipantePorFiltrosFachada fachadaFiltros = new ConsultarTipoParticipantePorFiltrosFachadaImpl();
			...
			ConsultarTipoParticipanteTodosFachada fachadaTodos = new ConsultarTipoParticipanteTodosFachadaImpl();
	...
	public ResponseEntity<RespuestaExito<Void>> registrar(@RequestBody TipoParticipanteDTO dto) {
		...
		return ResponseEntity.status(201).body(RespuestaExito.crear("Tipo de participante registrado exitosamente"));
	}
	...
	public ResponseEntity<RespuestaExito<Void>> eliminar(@PathVariable UUID id) {
		...
		fachada.ejecutar(dto);
		return ResponseEntity.ok(RespuestaExito.crear("Tipo de participante eliminado exitosamente"));
	}
}
