...
public class ParametroControlador {
	...
	public ResponseEntity<RespuestaExito<List<ParametroEntidad>>> consultar(
		...
		if (tieneFiltros) {
			ConsultarParametroPorFiltrosFachada fachadaFiltros = new ConsultarParametroPorFiltrosFachadaImpl();
	...
	public ResponseEntity<RespuestaExito<Void>> registrar(@RequestBody ParametroDTO dto) {
		...
		return ResponseEntity.status(201).body(RespuestaExito.crear("Parametro registrado exitosamente"));
	}
