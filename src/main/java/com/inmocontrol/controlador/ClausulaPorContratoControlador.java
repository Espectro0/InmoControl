...
public class ClausulaPorContratoControlador {
	...
	public ResponseEntity<RespuestaExito<List<ClausulaPorContratoEntidad>>> consultar(
		...
		if (tieneFiltros) {
			ConsultarClausulaPorContratoPorFiltrosFachada fachadaFiltros = new ConsultarClausulaPorContratoPorFiltrosFachadaImpl();
			...
			ConsultarClausulaPorContratoTodosFachada fachadaTodos = new ConsultarClausulaPorContratoTodosFachadaImpl();
	...
	@PostMapping
	public ResponseEntity<RespuestaExito<Void>> registrar(@RequestBody ClausulaPorContratoDTO dto) {
		RegistrarClausulaPorContratoFachada fachada = new RegistrarClausulaPorContratoFachadaImpl();
		fachada.ejecutar(dto);
		return ResponseEntity.status(201).body(RespuestaExito.crear("Clausula por contrato registrada exitosamente"));
	}
	...
	public ResponseEntity<RespuestaExito<Void>> eliminar(@PathVariable UUID id) {
		...
		fachada.ejecutar(dto);
		return ResponseEntity.ok(RespuestaExito.crear("Clausula por contrato eliminada exitosamente"));
	}
}
