...
public class AreaReferenciaControlador {
	...
	@GetMapping
	public ResponseEntity<RespuestaExito<List<AreaReferenciaEntidad>>> consultarTodos() {
		ConsultarAreaReferenciaTodosFachada fachada = new ConsultarAreaReferenciaTodosFachadaImpl();
		List<AreaReferenciaEntidad> resultado = fachada.ejecutar(null);
	...
	@GetMapping("/buscar")
	public ResponseEntity<RespuestaExito<List<AreaReferenciaEntidad>>> consultarPorFiltros(
			@RequestParam(required = false) String nombre) {
		AreaReferenciaDTO dto = new AreaReferenciaDTO.Builder().nombre(nombre).build();
		ConsultarAreaReferenciaPorFiltrosFachada fachada = new ConsultarAreaReferenciaPorFiltrosFachadaImpl();
		List<AreaReferenciaEntidad> resultado = fachada.ejecutar(dto);
		return ResponseEntity
				.ok(RespuestaExito.crear("Areas de referencia filtradas obtenidas exitosamente", resultado));
	}

	@PostMapping
	public ResponseEntity<RespuestaExito<AreaReferenciaEntidad>> registrar(@RequestBody AreaReferenciaDTO dto) {
		...
		return ResponseEntity.status(201).body(RespuestaExito.crear("Area de referencia registrada exitosamente", resultado));
	}
	...
	public ResponseEntity<RespuestaExito<Void>> eliminar(@PathVariable UUID id) {
		...
		fachada.ejecutar(dto);
		return ResponseEntity.ok(RespuestaExito.crear("Area de referencia eliminada exitosamente", null));
	}
}