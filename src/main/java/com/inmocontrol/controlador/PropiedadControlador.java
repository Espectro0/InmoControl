...
public class PropiedadControlador {
	...
	@GetMapping
	public ResponseEntity<RespuestaExito<List<PropiedadEntidad>>> consultarTodos() {
		ConsultarPropiedadTodosFachada fachada = new ConsultarPropiedadTodosFachadaImpl();
		List<PropiedadEntidad> resultado = fachada.ejecutar(null);
	...
	@GetMapping("/buscar")
	public ResponseEntity<RespuestaExito<List<PropiedadEntidad>>> consultarPorFiltros(
		...
		PropiedadDTO dto = new PropiedadDTO.Builder().nombreInmueble(nombreInmueble).direccion(direccion)
				...
				.build();
		ConsultarPropiedadPorFiltrosFachada fachada = new ConsultarPropiedadPorFiltrosFachadaImpl();
		List<PropiedadEntidad> resultado = fachada.ejecutar(dto);
	...
	public ResponseEntity<RespuestaExito<PropiedadEntidad>> registrar(@RequestBody PropiedadDTO dto) {
		...
		return ResponseEntity.status(201).body(RespuestaExito.crear("Propiedad registrada exitosamente", resultado));
	}
