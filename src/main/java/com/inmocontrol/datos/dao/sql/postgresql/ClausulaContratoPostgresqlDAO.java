...
public class ClausulaContratoPostgresqlDAO extends SQLDAO implements ClausulaContratoDAO {
	...
	public ClausulaContratoEntidad consultarPorId(UUID id) {
		String sql = "SELECT id, area_referencia_id, tipo_aplicacion_id, titulo, contenido_legal "
		...
		try (PreparedStatement stmt = getConexion().prepareStatement(sql)) {
			...
			throw new TransaccionExcepcion("Ocurrio un error al consultar la clausula contrato por id.", e);
		}
	...
	public List<ClausulaContratoEntidad> consultarPorFiltro(ClausulaContratoEntidad filtro) {
		...
		try (PreparedStatement stmt = getConexion().prepareStatement(sql)) {
			...
			throw new TransaccionExcepcion("Ocurrio un error al consultar clausulas contrato por filtro.", e);
		}
	...
	public void crear(ClausulaContratoEntidad entidad) {
		String sql = "INSERT INTO clausula_contrato (id, area_referencia_id, tipo_aplicacion_id, titulo, contenido_legal) "
				+ "VALUES (?, ?, ?, ?, ?)";

		try (PreparedStatement stmt = getConexion().prepareStatement(sql)) {
			stmt.setObject(1, entidad.getId());
			stmt.setObject(2, entidad.getAreaReferencia() != null ? entidad.getAreaReferencia().getId() : null);
			stmt.setObject(3, entidad.getTipoAplicacion() != null ? entidad.getTipoAplicacion().getId() : null);
	...
	public void actualizar(UUID id, ClausulaContratoEntidad entidad) {
		String sql = "UPDATE clausula_contrato SET area_referencia_id = ?, tipo_aplicacion_id = ?, "
				+ "titulo = ?, contenido_legal = ? WHERE id = ?";

		try (PreparedStatement stmt = getConexion().prepareStatement(sql)) {
			stmt.setObject(1, entidad.getAreaReferencia() != null ? entidad.getAreaReferencia().getId() : null);
			stmt.setObject(2, entidad.getTipoAplicacion() != null ? entidad.getTipoAplicacion().getId() : null);
	...
	private ClausulaContratoEntidad mapearResultado(ResultSet rs) throws SQLException {
		return new ClausulaContratoEntidad.Builder().id(rs.getObject("id", UUID.class))
				.areaReferencia(
						new AreaReferenciaEntidad.Builder().id(rs.getObject("area_referencia_id", UUID.class)).build())
				.tipoAplicacion(
						new TipoAplicacionEntidad.Builder().id(rs.getObject("tipo_aplicacion_id", UUID.class)).build())
				.titulo(rs.getString("titulo")).contenidoLegal(rs.getString("contenido_legal")).build();
	}
}