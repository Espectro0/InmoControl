...
public class ClausulaPorContratoPostgresqlDAO extends SQLDAO implements ClausulaPorContratoDAO {
	...
	public ClausulaPorContratoEntidad consultarPorId(UUID id) {
		String sql = "SELECT id, numero_clausula, contrato_id, clausula_id "
		...
		try (PreparedStatement stmt = getConexion().prepareStatement(sql)) {
			...
			throw new TransaccionExcepcion("Ocurrio un error al consultar la clausula por contrato por id.", e);
		}
	...
	public List<ClausulaPorContratoEntidad> consultarTodos() {
		...
		try (PreparedStatement stmt = getConexion().prepareStatement(sql)) {
			...
			throw new TransaccionExcepcion("Ocurrio un error al consultar las clausulas por contrato.", e);
		}
	...
	public List<ClausulaPorContratoEntidad> consultarPorFiltro(ClausulaPorContratoEntidad filtro) {
		String sql = "SELECT id, numero_clausula, contrato_id, clausula_id FROM clausula_por_contrato WHERE 1=1";
		...
		try (PreparedStatement stmt = getConexion().prepareStatement(sql)) {
			...
			throw new TransaccionExcepcion("Ocurrio un error al consultar clausulas por contrato por filtro.", e);
		}
	...
	public void crear(ClausulaPorContratoEntidad entidad) {
		String sql = "INSERT INTO clausula_por_contrato (id, numero_clausula, contrato_id, clausula_id) "
				+ "VALUES (?, ?, ?, ?)";
	...
	public void actualizar(UUID id, ClausulaPorContratoEntidad entidad) {
		String sql = "UPDATE clausula_por_contrato SET numero_clausula = ?, contrato_id = ?, "
	...
	private ClausulaPorContratoEntidad mapearResultado(ResultSet rs) throws SQLException {
		return new ClausulaPorContratoEntidad.Builder().id(rs.getObject("id", UUID.class))
				...
				.clausula(new ClausulaContratoEntidad.Builder().id(rs.getObject("clausula_id", UUID.class)).build())
				.build();
	}
}