...
public class ClausulaPorContratoPostgresqlDAO extends SQLDAO implements ClausulaPorContratoDAO {
	...
	public ClausulaPorContratoEntidad consultarPorId(UUID id) {
		String sql = "SELECT cpc.id, cpc.numeroclausula, cpc.contrato, cpc.clausula, "
				+ "cc.titulo, cc.contenidolegal, cc.areareferencia, cc.tipoaplicacion, "
				+ "ar.nombre as areareferencia_nombre, " + "ta.nombre as tipoaplicacion_nombre, "
				...
				+ "pr.areametros, pr.direccion, pr.ciudad as prop_ciudad, " + "tp.nombre as tipopropiedad_nombre, "
				...
				+ "ps.nombre as prop_pais_nombre " + "FROM clausulaporcontrato cpc "
				+ "JOIN clausulacontrato cc ON cpc.clausula = cc.id "
				+ "JOIN areareferencia ar ON cc.areareferencia = ar.id "
				+ "JOIN tipoaplicacion ta ON cc.tipoaplicacion = ta.id " + "JOIN contrato c ON cpc.contrato = c.id "
				+ "JOIN propiedad pr ON c.propiedad = pr.id " + "JOIN tipopropiedad tp ON pr.tipopropiedad = tp.id "
				+ "JOIN estrato e ON pr.estrato = e.id " + "JOIN ciudad ci ON pr.ciudad = ci.id "
				+ "JOIN departamento d ON ci.departamento = d.id " + "JOIN pais ps ON d.pais = ps.id "
				+ "WHERE cpc.id = ?";
		...
		try (PreparedStatement stmt = getConexion().prepareStatement(sql)) {
			...
			throw new InmocontrolExcepcion("No se pudo completar la operacion. Intente mas tarde.",
					"Error en ClausulaPorContratoPostgresqlDAO.consultarPorId() - " + e.getMessage(), e);
		}
	...
	public List<ClausulaPorContratoEntidad> consultarTodos() {
		...
		try (PreparedStatement stmt = getConexion().prepareStatement(sql)) {
			...
			throw new InmocontrolExcepcion("No se pudo completar la operacion. Intente mas tarde.",
					"Error en ClausulaPorContratoPostgresqlDAO.consultarTodos() - " + e.getMessage(), e);
		}
	...
	public List<ClausulaPorContratoEntidad> consultarPorFiltro(ClausulaPorContratoEntidad filtro) {
		String sql = "SELECT cpc.id, cpc.numeroclausula, cpc.contrato, cpc.clausula, "
				...
				+ "JOIN propiedad pr ON c.propiedad = pr.id " + "JOIN tipopropiedad tp ON pr.tipopropiedad = tp.id "
				+ "JOIN estrato e ON pr.estrato = e.id " + "JOIN ciudad ci ON pr.ciudad = ci.id "
				+ "JOIN departamento d ON ci.departamento = d.id " + "JOIN pais ps ON d.pais = ps.id " + "WHERE 1=1";
		List<Object> parametros = new ArrayList<>();

		if (filtro.getNumeroClausula() != UtilNumero.ENTERO_DEFECTO) {
		...
		try (PreparedStatement stmt = getConexion().prepareStatement(sql)) {
			...
			throw new InmocontrolExcepcion("No se pudo completar la operacion. Intente mas tarde.",
					"Error en ClausulaPorContratoPostgresqlDAO.consultarPorFiltro() - " + e.getMessage(), e);
		}
	...
	public void crear(ClausulaPorContratoEntidad entidad) {
		String sql = "INSERT INTO clausulaporcontrato (id, numeroclausula, contrato, clausula) "
				+ "VALUES (?, ?, ?, ?)";
		...
		try (PreparedStatement stmt = getConexion().prepareStatement(sql)) {
			...
			throw new InmocontrolExcepcion("No se pudo completar la operacion. Intente mas tarde.",
					"Error en ClausulaPorContratoPostgresqlDAO.crear() - " + e.getMessage(), e);
		}
	}

	@Override
	public void actualizar(UUID id, ClausulaPorContratoEntidad entidad) {
		String sql = "UPDATE clausulaporcontrato SET numeroclausula = ?, contrato = ?, " + "clausula = ? WHERE id = ?";
		...
		try (PreparedStatement stmt = getConexion().prepareStatement(sql)) {
			...
			throw new InmocontrolExcepcion("No se pudo completar la operacion. Intente mas tarde.",
					"Error en ClausulaPorContratoPostgresqlDAO.actualizar() - " + e.getMessage(), e);
		}
	}
	...
	public void eliminar(UUID id) {
		...
		try (PreparedStatement stmt = getConexion().prepareStatement(sql)) {
			...
			throw new InmocontrolExcepcion("No se pudo completar la operacion. Intente mas tarde.",
					"Error en ClausulaPorContratoPostgresqlDAO.eliminar() - " + e.getMessage(), e);
		}
	}

	private ClausulaPorContratoEntidad mapearResultado(ResultSet rs) throws SQLException {
		return new ClausulaPorContratoEntidad.Builder().id(rs.getObject("id", UUID.class))
				.numeroClausula(rs.getObject("numeroclausula", Integer.class))
				.clausula(
						new ClausulaContratoEntidad.Builder().id(rs.getObject("clausula", UUID.class))
								.titulo(rs.getString("titulo")).contenidoLegal(rs.getString("contenidolegal"))
								.areaReferencia(new AreaReferenciaEntidad.Builder()
										.id(rs.getObject("areareferencia", UUID.class))
										.nombre(rs.getString("areareferencia_nombre")).build())
								.tipoAplicacion(new TipoAplicacionEntidad.Builder()
										.id(rs.getObject("tipoaplicacion", UUID.class))
										.nombre(rs.getString("tipoaplicacion_nombre")).build())
								.build())
				.contrato(
						new ContratoEntidad.Builder().id(rs.getObject("contrato", UUID.class))
								...
								.propiedad(new com.inmocontrol.entidad.PropiedadEntidad.Builder()
										.id(rs.getObject("propiedad", UUID.class))
										.tipoPropiedad(new TipoPropiedadEntidad.Builder()
										...
										.estrato(new EstratoEntidad.Builder().id(rs.getObject("estrato", UUID.class))
												...
												.descripcion(rs.getString("estrato_descripcion")).build())
										...
										.ciudad(new CiudadEntidad.Builder().id(rs.getObject("prop_ciudad", UUID.class))
												.nombre(rs.getString("prop_ciudad_nombre"))
												.departamento(new DepartamentoEntidad.Builder()
														.id(rs.getObject("prop_ciudad_departamento", UUID.class))
														.nombre(rs.getString("prop_departamento_nombre"))
														.pais(new PaisEntidad.Builder()
																.id(rs.getObject("prop_departamento_pais", UUID.class))
																.nombre(rs.getString("prop_pais_nombre")).build())
														.build())
												.build())
										.build())
								.build())
