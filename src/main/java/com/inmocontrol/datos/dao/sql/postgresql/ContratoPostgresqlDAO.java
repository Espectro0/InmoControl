...
public class ContratoPostgresqlDAO extends SQLDAO implements ContratoDAO {
  ...
  public ContratoEntidad consultarPorId(UUID id) {
    String sql =
        "SELECT c.id, c.codigocontrato, c.fechainicio, c.fechafin, c.esactivo, c.propiedad, "
            ...
            + "JOIN ciudad ci ON p.ciudad = ci.id "
  ...
  public void crear(ContratoEntidad entidad) {
    String sql =
        "INSERT INTO contrato (id, codigocontrato, fechainicio, fechafin, esactivo, propiedad) "
            + "VALUES (?, ?, ?, ?, ?, ?)";
  ...
  public void actualizar(UUID id, ContratoEntidad entidad) {
    String sql =
        "UPDATE contrato SET codigocontrato = ?, fechainicio = ?, fechafin = ?, "
