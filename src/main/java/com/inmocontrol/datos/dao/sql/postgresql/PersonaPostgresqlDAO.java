...
public class PersonaPostgresqlDAO extends SQLDAO implements PersonaDAO {
	...
	public PersonaEntidad consultarPorId(UUID id) {
		String sql = "SELECT id, tipo_documento_id, numero_identificacion, primer_nombre, "
	...
	public void crear(PersonaEntidad entidad) {
		String sql = "INSERT INTO persona (id, tipo_documento_id, numero_identificacion, primer_nombre, "
				+ "segundo_nombre, primer_apellido, segundo_apellido, numero_telefonico, "
				+ "correo_electronico, direccion_residencia, ciudad_residencia_id, "
		...
		try (PreparedStatement stmt = getConexion().prepareStatement(sql)) {
			...
			stmt.setObject(2, entidad.getTipoDocumento() != null ? entidad.getTipoDocumento().getId() : null);
			...
			stmt.setObject(11, entidad.getCiudadResidencia() != null ? entidad.getCiudadResidencia().getId() : null);
			stmt.setDate(12,
					entidad.getFechaNacimiento() != null ? new Date(entidad.getFechaNacimiento().getTime()) : null);
	...
	public void actualizar(UUID id, PersonaEntidad entidad) {
		String sql = "UPDATE persona SET tipo_documento_id = ?, numero_identificacion = ?, primer_nombre = ?, "
				+ "segundo_nombre = ?, primer_apellido = ?, segundo_apellido = ?, numero_telefonico = ?, "
				+ "correo_electronico = ?, direccion_residencia = ?, ciudad_residencia_id = ?, "
				+ "fecha_nacimiento = ?, edad = ? WHERE id = ?";

		try (PreparedStatement stmt = getConexion().prepareStatement(sql)) {
			stmt.setObject(1, entidad.getTipoDocumento() != null ? entidad.getTipoDocumento().getId() : null);
			...
			stmt.setObject(10, entidad.getCiudadResidencia() != null ? entidad.getCiudadResidencia().getId() : null);
			stmt.setDate(11,
					entidad.getFechaNacimiento() != null ? new Date(entidad.getFechaNacimiento().getTime()) : null);
	...
	private PersonaEntidad mapearResultado(ResultSet rs) throws SQLException {
		return new PersonaEntidad.Builder().id(rs.getObject("id", UUID.class))
				.tipoDocumento(
						new TipoDocumentoEntidad.Builder().id(rs.getObject("tipo_documento_id", UUID.class)).build())
				.numeroIdentificacion(rs.getString("numero_identificacion")).primerNombre(rs.getString("primer_nombre"))
				.segundoNombre(rs.getString("segundo_nombre")).primerApellido(rs.getString("primer_apellido"))
				.segundoApellido(rs.getString("segundo_apellido")).numeroTelefonico(rs.getString("numero_telefonico"))
				.correoElectronico(rs.getString("correo_electronico"))
				.direccionResidencia(rs.getString("direccion_residencia"))
				.ciudadResidencia(
						new CiudadEntidad.Builder().id(rs.getObject("ciudad_residencia_id", UUID.class)).build())
				.fechaNacimiento(rs.getDate("fecha_nacimiento")).edad(rs.getObject("edad", Integer.class)).build();
	}
}