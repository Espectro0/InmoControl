...
public class PropiedadPostgresqlDAO extends SQLDAO implements PropiedadDAO {
	...
	public PropiedadEntidad consultarPorId(UUID id) {
		String sql = "SELECT id, tipo_propiedad_id, estrato_id, nombre_inmueble, "
	...
	public void crear(PropiedadEntidad entidad) {
		String sql = "INSERT INTO propiedad (id, tipo_propiedad_id, estrato_id, nombre_inmueble, "
				+ "descripcion_inmueble, area_metros, direccion, ciudad_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

		try (PreparedStatement stmt = getConexion().prepareStatement(sql)) {
			stmt.setObject(1, entidad.getId());
			stmt.setObject(2, entidad.getTipoPropiedad() != null ? entidad.getTipoPropiedad().getId() : null);
	...
	public void actualizar(UUID id, PropiedadEntidad entidad) {
		String sql = "UPDATE propiedad SET tipo_propiedad_id = ?, estrato_id = ?, nombre_inmueble = ?, "
				+ "descripcion_inmueble = ?, area_metros = ?, direccion = ?, ciudad_id = ? WHERE id = ?";

		try (PreparedStatement stmt = getConexion().prepareStatement(sql)) {
			stmt.setObject(1, entidad.getTipoPropiedad() != null ? entidad.getTipoPropiedad().getId() : null);
	...
	private PropiedadEntidad mapearResultado(ResultSet rs) throws SQLException {
		return new PropiedadEntidad.Builder().id(rs.getObject("id", UUID.class))
				.tipoPropiedad(
						new TipoPropiedadEntidad.Builder().id(rs.getObject("tipo_propiedad_id", UUID.class)).build())
				...
				.areaMetros(rs.getObject("area_metros", Integer.class)).direccion(rs.getString("direccion"))
				.ciudad(new CiudadEntidad.Builder().id(rs.getObject("ciudad_id", UUID.class)).build()).build();
	}
}