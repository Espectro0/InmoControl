...
public class ConsultarAreaReferenciaPorFiltrosFachadaImpl implements ConsultarAreaReferenciaPorFiltrosFachada {
	...
	public List<AreaReferenciaEntidad> ejecutar(AreaReferenciaDTO datos) {
		if (UtilObjeto.esNulo(datos)) {
			throw new InmocontrolExcepcion("Los datos del area de referencia no pueden ser nulos",
					"Validacion fallida en ConsultarAreaReferenciaPorFiltrosFachadaImpl.ejecutar() - Los datos del area de referencia no pueden ser nulos");
		}

		try {
			AreaReferenciaDominio dominio = new AreaReferenciaDominio.Builder().nombre(datos.getNombre()).build();
			return casoUso.ejecutar(dominio);

		} catch (Exception excepcion) {
			throw new InmocontrolExcepcion("No se pudo completar la operacion. Intente mas tarde.",
					"Error en ConsultarAreaReferenciaPorFiltrosFachadaImpl.ejecutar() - " + excepcion.getMessage(),
					excepcion);
