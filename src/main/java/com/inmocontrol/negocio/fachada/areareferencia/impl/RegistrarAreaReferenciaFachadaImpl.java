	...
	public void ejecutar(AreaReferenciaDTO datos) {
		if (UtilObjeto.esNulo(datos)) {
			throw new InmocontrolExcepcion("Los datos del area de referencia no pueden ser nulos",
					"Validacion fallida en RegistrarAreaReferenciaFachadaImpl.ejecutar() - Los datos del area de referencia no pueden ser nulos");
		}
		...
		try {
			...
			throw new InmocontrolExcepcion("No se pudo completar la operacion. Intente mas tarde.",
					"Error en RegistrarAreaReferenciaFachadaImpl.ejecutar() - " + excepcion.getMessage(), excepcion);
