...
public class ConsultarTipoPropiedadPorFiltrosFachadaImpl implements ConsultarTipoPropiedadPorFiltrosFachada {
	...
	public List<TipoPropiedadEntidad> ejecutar(TipoPropiedadDTO datos) {
		if (UtilObjeto.esNulo(datos)) {
			throw new InmocontrolExcepcion("Los datos del tipo de propiedad no pueden ser nulos",
					"Validacion fallida en ConsultarTipoPropiedadPorFiltrosFachadaImpl.ejecutar() - Los datos del tipo de propiedad no pueden ser nulos");
		}

		try {
			TipoPropiedadDominio dominio = new TipoPropiedadDominio.Builder().nombre(datos.getNombre()).build();
			return casoUso.ejecutar(dominio);

		} catch (Exception excepcion) {
			throw new InmocontrolExcepcion("No se pudo completar la operacion. Intente mas tarde.",
					"Error en ConsultarTipoPropiedadPorFiltrosFachadaImpl.ejecutar() - " + excepcion.getMessage(),
					excepcion);
