...
public class RegistrarPropiedadCasoUsoImpl implements RegistrarPropiedadCasoUso {
	...
	private void validarObligatoriedadCampos(PropiedadDominio datos) {
		...
		if (UtilObjeto.esNulo(datos.getTipoPropiedad()) || UtilObjeto.esNulo(datos.getTipoPropiedad().getId())) {
	...
	private void registrarPropiedad(PropiedadDominio datos) {
		PropiedadEntidad entidad = new PropiedadEntidad.Builder()
				.tipoPropiedad(new TipoPropiedadEntidad.Builder().id(datos.getTipoPropiedad().getId()).build())
				.estrato(
						datos.getEstrato() != null ? new EstratoEntidad.Builder().id(datos.getEstrato().getId()).build()
								: null)
				...
				.ciudad(new CiudadEntidad.Builder().id(datos.getCiudad().getId()).build()).build();
