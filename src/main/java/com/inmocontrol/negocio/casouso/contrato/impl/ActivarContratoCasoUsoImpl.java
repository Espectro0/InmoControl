...
public class ActivarContratoCasoUsoImpl implements ActivarContratoCasoUso {
	...
	private void validarObligatoriedadId(ContratoDominio datos) {
		if (UtilObjeto.esNulo(datos)) {
			throw new InmocontrolExcepcion("El contrato a activar no es valido.",
	...
	private void validarExistenciaContrato(ContratoDominio datos) {
		...
		if (UtilObjeto.esNulo(existente)) {
			throw new InmocontrolExcepcion("No existe un contrato con el ID: " + datos.getId(),
					"Error en ActivarContratoCasoUsoImpl.validarExistenciaContrato() - No existe un contrato con el ID: "
							+ datos.getId());
		}
	}
	...
	private void validarQueEstaSuspendido(ContratoDominio datos) {
		...
		if (Boolean.TRUE.equals(existente.getEsActivo())) {
			throw new InmocontrolExcepcion("El contrato ya se encuentra activo.",
	...
	private void validarTieneClausulas(ContratoDominio datos) {
		ClausulaPorContratoEntidad filtro = new ClausulaPorContratoEntidad.Builder()
				.contrato(new ContratoEntidad.Builder().id(datos.getId()).build()).build();
		var clausulas = daoFactory.obtenerClausulaPorContratoDAO().consultarPorFiltro(filtro);
		if (clausulas == null || clausulas.isEmpty()) {
			throw new InmocontrolExcepcion("El contrato debe tener al menos una clausula para poder ser activado.",
	...
	private void activarContrato(ContratoDominio datos) {
		ContratoEntidad existente = daoFactory.obtenerContratoDAO().consultarPorId(datos.getId());
		ContratoEntidad entidad = new ContratoEntidad.Builder().id(existente.getId())
				.codigoContrato(existente.getCodigoContrato()).fechaInicio(existente.getFechaInicio())
				.fechaFin(existente.getFechaFin()).esActivo(true).propiedad(existente.getPropiedad()).build();
