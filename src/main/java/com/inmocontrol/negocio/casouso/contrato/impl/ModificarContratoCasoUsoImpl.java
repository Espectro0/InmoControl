  ...
  private void validarObligatoriedadId(ContratoDominio datos) {
    if (UtilObjeto.esNulo(datos)) {
      throw new InmocontrolExcepcion(
          "El contrato a modificar no es valido.",
  ...
  private void validarExistenciaContrato(ContratoDominio datos) {
    ...
    if (UtilObjeto.esNulo(existente)) {
      throw new InmocontrolExcepcion(
          ...
          "Error en ModificarContratoCasoUsoImpl.validarExistenciaContrato() - No existe un contrato con el ID: " + datos.getId()
      );
    }
  }
    ...
    for (ContratoEntidad item : resultados) {
      if (!item.getId().equals(datos.getId())) {
        throw new InmocontrolExcepcion(
            ...
            "Validacion fallida en ModificarContratoCasoUsoImpl.validarUnicoCodigoContrato() - Ya existe un contrato con el codigo: " + datos.getCodigoContrato()
        );
      }
    }
  }
